...
3. **The Control Plane (`ginnungagap` CLI/Daemon):** 
   The Ginnungagap application acts as a control plane, directly parsing Falco's real-time logs 
(bypassing the HTTP overhead of Falco Sidekick) to win the "race condition" against malware. 
The precise millisecond it detects a rule violation generated by the sandbox:
   - It immediately uses the Docker Engine API (via `/var/run/docker.sock`) to issue a **`docker pause`** command, 
freezing the sandbox container in its tracks before the malware can finish its execution or delete itself.
   - It then runs **`docker commit`** and exports the frozen state into a `.tar.gz` archive (password protected).
   - Finally, the CLI notifies you with a detailed forensic report, 
leaving you with the `.tar.gz` artifact to analyze, submit as a CVE, or share with the community.
